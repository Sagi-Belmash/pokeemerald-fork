...
[כללים לתרגום](#כללים-לתרגום)
  ...
  + [מספרים ותוווים מיוחדים](#מספרים-ותווים-מיוחדים)
...
יש במשחק התייחסויות לזכר ונקבה בעזרת ממלאי מקום במקום להוסיף עוד תנאים ותיבות שיח. ההתייחסות היא רק כשפונים לשחקן או שחקנית.
...
Examples:
* ADD_YUD - תלמד{ADD_YUD} - תלמד לבן ותלמדי לבת
* ADD_HEY - ילד{ADD_HEY} - ילד לבן וילדה לבת
* ADD_TAF - נועל{ADD_TAF} - נועל לבן ונועלת לבת
* EMIT_YUD - 
* EMIT_HEY - את{EMIT_HEY} - אתה לבן ואת לבת
* EMIT_TAF - נרגש{EMIT_TAF} - נרגשת ליריבה ונרגש ליריב
* EMIT_VAV - תשמ{EMIT_VAV}ר{ADD_YUD} - תשמור לבן ותשמרי לבת
* SWAP_CHAF - ל{SWAP_CHAF}{ADD_YUD} - לך לבן ולכי לבת
* SWAP_MEM - תילח{SWAP_MEM}{ADD_YUD} - תילחם לבן ותילחמי לבת
* SWAP_NUN - ת{SWAP_NUN}{ADD_YUD} - תן לבן ותני לבת
* SWAP_PEY - החלי{SWAP_PEY}{ADD_HEY} - החליף לבן והחליפה לבת
* SWAP_TZADIK - תתאמ{SWAP_TZADIK}{ADD_YUD} - תתאמץ לבן ותתאמצי לבת
</div>
...
יש להשאיר את התו … כמו שהוא ולא להחליפו בשלוש נקודות.\
מרכאות יש להפוך את הסדר שלהם, כלומר הפותח נהיה הסוגר ולהפך: למשל “STORAGE” הופך ל- ”אחסון“\
עבור שמות של פריטים יש להשתמש בשמות מ- src/data/items.h
